"use client";

import { useEffect, useState } from "react";
import { connectSocket, onIncomingCall } from "@/lib/socket";
import { apiFetch } from "@/lib/utils";
import type { IncomingCallEvent } from "@samvedna/shared-types";
import { Button } from "@/components/ui/button";
import { Phone, X } from "lucide-react";

export function IncomingCallPanel({
  userId,
  token,
  onRefresh,
}: {
  userId: string;
  token: string;
  onRefresh: () => void;
}) {
  const [calls, setCalls] = useState<IncomingCallEvent[]>([]);

  useEffect(() => {
    connectSocket(userId);
    return onIncomingCall((event) => {
      setCalls((prev) => [event, ...prev.filter((c) => c.call_session.id !== event.call_session.id)].slice(0, 3));
    });
  }, [userId]);

  async function accept(id: string) {
    await apiFetch(`/calls/${id}/status`, {
      method: "PATCH",
      token,
      body: JSON.stringify({ status: "in_progress" }),
    });
    setCalls((p) => p.filter((c) => c.call_session.id !== id));
    onRefresh();
  }

  if (!calls.length) return null;

  return (
    <div className="fixed bottom-4 left-4 z-50 flex max-w-md flex-col gap-2">
      {calls.map((c) => (
        <div
          key={c.call_session.id}
          className="rounded-lg border border-risk-high/40 bg-card p-4 shadow-xl"
        >
          <div className="flex items-start gap-3">
            <Phone className="mt-0.5 h-5 w-5 shrink-0 animate-pulse text-risk-high" />
            <div className="flex-1 text-sm">
              <p className="font-semibold">Incoming counsellor call</p>
              <p className="text-muted-foreground">{c.victim_name} · {c.case_number}</p>
              <p className="mt-1 text-xs capitalize">Risk: {c.call_session.risk_level_at_call}</p>
              <div className="mt-3 flex gap-2">
                <Button size="sm" onClick={() => accept(c.call_session.id)}>
                  Accept
                </Button>
                <Button size="sm" variant="outline" asChild>
                  <a href="/counsellor/calls">Open calls</a>
                </Button>
              </div>
            </div>
            <button onClick={() => setCalls((p) => p.filter((x) => x.call_session.id !== c.call_session.id))}>
              <X className="h-4 w-4 text-muted-foreground" />
            </button>
          </div>
        </div>
      ))}
    </div>
  );
}
